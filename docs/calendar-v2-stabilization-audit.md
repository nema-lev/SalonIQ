...
| Sample mode vs real mode | Sample mode disables backend reads/writes; real mode is default; query param controls the switch. | Write safety is strong. UX risk remains because sample affordances are available directly from real empty/error states and the UI still talks about sample concepts. |
...
| --- | --- | --- |
| `native-scheduler-spike` naming | The naming remains in internal folder/file names and internal console labels. The real adapter overrides the visible toolbar eyebrow to `Calendar V2`, so the inspected normal UI does not expose the spike name directly. | Not a blocker by itself, but it signals that some implementation vocabulary is still prototype-era. |
| Preview/read-only wording | Sample mode is correctly labeled read-only. In real mode, placement and reschedule previews still say `Само преглед` because the action is not committed yet, which is semantically defensible. | Not a correctness blocker, but the product still carries preview-heavy language and should be tightened before broad operator use. |
| Sample/demo labels in real mode | Real empty/error states expose `Show sample day`; real mode can surface `Sample staff names`; sample mode is one click away from the live route. | **Pilot polish issue.** Ordinary salon operators should not see internal sample concepts in routine real-mode use. |
| English copy in real operational states | Examples include `Calendar data is unavailable.`, `Retry read`, `Show sample day`, `Loading calendar data`, `No staff available for this date`, and `No bookings scheduled for this date`. | **Pilot polish issue.** Calendar actions are Bulgarian, but several surrounding trust states are still English. |
