...
- Request placement persistence remains disabled: the preview creates a typed `placeRequest` command object with `localOnly: true`, but no appointment, waitlist, status, notification, or reschedule API is called.
...
- A future dedicated backend placement endpoint is still required before Calendar V2 can save request placement.
