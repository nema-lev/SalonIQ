...
| Beta/admin testing calendar | **Not yet ready for broad beta** | It is suitable for controlled internal testing of viewing, manual booking creation, eligible booking confirmation/cancellation, explicit click-to-reschedule, and request placement. Error recovery is still too shallow for confident multi-admin testing. |
...
| Reschedule/move booking | Yes: modal and drag flow | Yes: explicit Booking Detail → `Премести час` → future slot → preview → save flow; drag/drop persistence remains disabled |
...
Today an owner can **see** the day, inspect bookings, create a direct manual booking on desktop real mode, confirm an eligible pending/proposal timed booking, move an eligible real timed booking through an explicit click-to-reschedule flow, cancel an eligible real-data booking from Booking Detail, and optionally place an untimed request if the feature flag is enabled. Cancelled bookings no longer remain as active blocking cards in the Calendar V2 grid; they can still surface as non-blocking updates/history items.

That means Calendar V2 is the primary route, but it is not yet the only calendar surface an operator can rely on because phone flow, stronger recovery, and the remaining data-hardening work are still incomplete.
...
2. Fix structured error-state and post-commit refresh/reconciliation behavior for the currently enabled save paths.
3. Hide or gate sample/demo production wording from ordinary real-mode operator experience.
...
Calendar V2 now supports confirming eligible `pending` / `proposal_pending` timed bookings and cancelling eligible bookings from Booking Detail by reusing `PATCH /appointments/:id/status`, plus explicit click-to-reschedule for eligible `pending`, `proposal_pending`, and `confirmed` real timed appointments through the existing `PATCH /appointments/:id/reschedule` endpoint with `{ startAt, staffId }`. For standard appointments, the existing backend status lifecycle promotes the matching held `calendar_allocations` row to booked when the booking is confirmed, deactivates/releases it when terminal statuses are applied, and keeps reschedule updates plus matching allocation changes atomic. The existing status endpoint still runs its current status-change/cancellation notification behavior; the reschedule endpoint does not add a notification path. Sample mode remains non-writing. Drag/drop persistence remains disabled.

The next production-hardening work after the owner-usability P0 is mobile flow, structured conflict/recovery, backfill/report execution, and realtime later.
