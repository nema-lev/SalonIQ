...
- Calendar V2 appointment cards were simplified toward a block-like zero-training model: grid cards now keep client, service, exact time, and only useful cues; confirmed/default bookings do not render loud status text; pending/proposal bookings expose action-needed display metadata; short cards keep client identity plus time; duration text is left off the card because block height carries duration.
...
- Remaining UX hardening after the appointment-card pass: strengthen free-slot affordance, make request placement/reschedule previews feel more like physical ghost blocks, simplify the selected-detail action hierarchy, and later replace the legacy manual-booking modal entry with a Calendar V2-native draft-block flow.
