...
describe('AppointmentsService.placeWaitlistEntry', () => {
  it('creates an appointment and marks the waitlist row booked in one tenant transaction', async () => {
    const tx = {
      $queryRawUnsafe: jest
        .fn()
  ...
  it('rejects a conflicting appointment for the same staff member', async () => {
    const tx = {
      $queryRawUnsafe: jest
        .fn()
        .mockResolvedValueOnce([openWaitlist])
